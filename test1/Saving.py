...
def save_money_in_n_weeks(money_per_week, increase_money, total_week):
    ...
    return saved_money_list, money_list
...
def main():
    ...
    # 调用函数
    saved_money_list, money_list = save_money_in_n_weeks(money_per_week, increase_money, total_week)

    input_date_str = input('请输入日期(yyyy/mm/dd):')
    input_date = datetime.datetime.strptime(input_date_str, '%Y/%m/%d')
    week_num = input_date.isocalendar()[1]
    print('第{}周的存款：{}元，账户累计：{}元'.format(week_num, money_list[week_num - 1], saved_money_list[
        week_num - 1]))
